...
const useEditUser = (data: IUsers) => {
  ...
  const initialFormValues: IGeneralUserFormValues = {
    generalInformationStep: {
      ...
      values: {
        ...
        birthDate: formatDate(new Date(data?.birthDay)) || "",
      },
    },
    missionForStaffStep: {
      isValid: true,
      values: {
        missionValue: data?.missionData?.missionName || "",
        missionName: data?.missionData?.missionName || "",
        missionDescription: data?.missionData?.descriptionUse || "",
      },
    },
    contactDataStep: {
      isValid: true,
    ...
    businessUnitsStep: data.businessUnitsAsigned.map((businessUnit) => ({
      id: businessUnit.businessUnitCode,
      value: businessUnit.businessUnitName,
      isActive: true,
    })),
    positionByBusinessUnitStep: [
      data.businessUnitsAsigned.reduce(
        (acc, businessUnit) => ({
          ...acc,
          [businessUnit.businessUnitCode]: {
            options: [
              {
                id: businessUnit.positionName,
                label: businessUnit.positionName,
                value: businessUnit.positionName,
              },
            ],
            value: businessUnit.positionName,
          },
    ...
    roleByBusinessUnitStep: data.businessUnitsAsigned.flatMap((businessUnit) =>
      businessUnit.roles.map((role) => ({
        id: role.roleName,
        value: role.roleName,
        isActive: true,
        businessUnitCode: businessUnit.businessUnitCode,
        positionName: businessUnit.positionName,
      })),
    ),
  };

  const [formValues, setFormValues] =
    useState<IGeneralUserFormValues>(initialFormValues);
  ...
  const handleNextStep = () => {
    ...
    if (
      currentTab === editUserTabsConfig.generalInformation.id &&
    ...
    if (
      currentTab === editUserTabsConfig.missionForStaff.id &&
      missionForStaffRef.current
    ) {
      setFormValues((prev) => ({
        ...prev,
        missionForStaffStep: {
    ...
    if (
      currentTab === editUserTabsConfig.contactData.id &&
  ...
  const onSubmit = () => {
    const general =
      ...
      formValues.generalInformationStep.values;
    const mission =
      missionForStaffRef.current?.values ||
      formValues.missionForStaffStep.values;
    ...
    setSaveData({
      ...
      configurationRequestData: {
        ...
        missionData: {
          missionName: mission?.missionValue || "",
        },
        missionName: mission?.missionValue || "",
      },
    });
  ...
  const showMissionForStaff =
    isSelected === editUserTabsConfig.missionForStaff.id;
  ...
  const showPositionByBusinessUnit =
    isSelected === editUserTabsConfig.positionByBusinessUnit.id;
  const showRoleByBusinessUnit =
    isSelected === editUserTabsConfig.roleByBusinessUnit.id;
